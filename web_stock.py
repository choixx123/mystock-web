...
# --- 보조지표 연산 (기존과 완벽히 동일) ---
def calc_ma(prices, window): return [None if i < window - 1 else sum(prices[i-window+1:i+1])/window for i in range(len(prices))]
def calc_std(prices, window, ma):
    std = []
    for i in range(len(prices)):
        if i < window - 1 or ma[i] is None: std.append(None)
        else: std.append(math.sqrt(sum((p - ma[i])**2 for p in prices[i-window+1:i+1])/window))
    return std
...
def calc_macd(prices):
    ...
    macd = [e12 - e26 if e12 is not None and e26 is not None else None for e12, e26 in zip(ema12, ema26)]
...
with st.sidebar:
    ...
    st.markdown("---")
    st.caption("CEO 터미널 V12.0 (구역 분리 완벽 최적화)")
...
# 1️⃣ 종목 심볼(Ticker) 먼저 찾기 (두 구역이 공통으로 사용)
...
if not symbol:
    st.error(f"❌ '{original_name}' 정보가 없거나 야후 서버가 응답하지 않습니다. (검색어를 확인해주세요)")
    st.stop() # 검색 실패 시 여기서 정지!

# =========================================================
# 🚀 [구역 1: 초고속 라이브 구역] 숫자만 5초마다 깜빡임 없이 갱신!
# =========================================================
...
def render_live_metrics(target_symbol, target_name):
    ...
    for col, (name, sym, sign) in zip([m1, m2, m3, m4, m5], indices):
        p, pct = get_quick_quote(sym)
        with col:
    ...
    # 선택한 주식의 현재가 및 KPI 정보 (1년치 데이터로 52주 고/저점 계산)
    res_1y_data = get_cached_json(f"https://query1.finance.yahoo.com/v8/finance/chart/{target_symbol}?range=1y&interval=1d")
    if not res_1y_data or 'chart' not in res_1y_data: return st.error("❌ 주가 데이터를 불러올 수 없습니다.")
...
if chart_res_json and chart_res_json['chart']['result']:
    ...
    for i in range(len(dt_objects)):
        if i < len(closes) and closes[i] is not None:
            ...
            clean_data.append((dt_objects[i], opens[i] if i<len(opens) else closes[i], highs[i] if i<len(highs) else closes[i], lows[i] if i<len(lows) else closes[i], closes[i], v))

    full_prices = [row[4] for row in clean_data]
    ma20_full, ma60_full = calc_ma(full_prices, 20), calc_ma(full_prices, 60)
    ...
    bb_upper_full = [m + 2*s if m and s else None for m, s in zip(ma20_full, std_full)]
    bb_lower_full = [m - 2*s if m and s else None for m, s in zip(ma20_full, std_full)]
    ...
    if timeframe == "1일" and len(clean_data) > 0:
        ...
        for i in range(len(clean_data) - 1, 0, -1):
            if (clean_data[i][0] - clean_data[i-1][0]).total_seconds() > 4 * 3600: 
                session_start_idx = i; break
        for i in range(session_start_idx, len(clean_data)):
            f_dates.append(clean_data[i][0]); f_opens.append(clean_data[i][1]); f_highs.append(clean_data[i][2]); f_lows.append(clean_data[i][3]); f_closes.append(clean_data[i][4]); f_volumes.append(clean_data[i][5])
            f_ma20.append(ma20_full[i]); f_ma60.append(ma60_full[i]); f_rsi.append(rsi_full[i]); f_macd.append(macd_full[i]); f_signal.append(macd_signal_full[i]); f_bb_up.append(bb_upper_full[i]); f_bb_dn.append(bb_lower_full[i])
    ...
    if use_candle and len(f_dates) > 0:
        fig.add_trace(go.Candlestick(x=f_dates, open=f_opens, high=f_highs, low=f_lows, close=f_closes, increasing_line_color=up_color, decreasing_line_color=down_color, name='캔들'), row=1, col=1, secondary_y=False)
    elif len(f_dates) > 0:
        fig.add_trace(go.Scatter(x=f_dates, y=f_closes, mode='lines', name='주가', line=dict(color='#00b4d8', width=3)), row=1, col=1, secondary_y=False)

    if use_bb and len(f_dates) > 0:
        fig.add_trace(go.Scatter(x=f_dates, y=f_bb_up, mode='lines', name='볼린저 상단', line=dict(color='rgba(173, 216, 230, 0.5)', width=1)), row=1, col=1)
        fig.add_trace(go.Scatter(x=f_dates, y=f_bb_dn, mode='lines', name='볼린저 하단', fill='tonexty', fillcolor='rgba(173, 216, 230, 0.1)', line=dict(color='rgba(173, 216, 230, 0.5)', width=1)), row=1, col=1)
    ...
    vol_colors = [up_color if i==0 or f_closes[i]>=f_closes[i-1] else down_color for i in range(len(f_closes))]
    if len(f_dates) > 0:
        ...
        if bottom_indicator == "RSI":
            ...
            hist_colors = ['#ff4b4b' if (m and s and m>s) else '#00b4d8' for m, s in zip(f_macd, f_signal)]
            fig.add_trace(go.Bar(x=f_dates, y=[m-s if m and s else 0 for m,s in zip(f_macd, f_signal)], marker_color=hist_colors), row=2, col=1)

    fig.update_layout(title=f"📈 {official_name} 차트 & 보조지표", hovermode="x unified", height=700, margin=dict(l=0, r=0, t=40, b=0), xaxis_rangeslider_visible=False)
